/**
 * icons.jsx — SVG icon set + <Icon> renderer
 * ==========================================
 * The icon markup is copied verbatim from the original configs.js so the
 * sidebar and stat cards look identical. Icons are trusted static strings, so
 * we render them with dangerouslySetInnerHTML.
 */
...
export const ICONS = {
  ...
  search: '<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="M21 21l-4.3-4.3"/></svg>',
  menu: '<svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 6h18M3 12h18M3 18h18"/></svg>',
  chevronLeft: '<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2"><path d="M15 18l-6-6 6-6"/></svg>',
};

/** Render a named icon (or raw SVG string) as inline SVG. */
export function Icon({ name, svg, className }) {
  const markup = svg || ICONS[name] || ICONS.grid;
